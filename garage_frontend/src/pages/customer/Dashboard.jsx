    ...
    useEffect(() => {
        const fetchBookings = async () => {
            try {
                ...
                // Find next upcoming (Approved or In Progress)
                const next = bookings
        ...
        <div className={styles.container}>
            ...
            {upcoming ? (
                <div className={styles.cardBlue}>
                    <h2 className={styles.cardTitle} style={{ color: '#1e3a8a', fontSize: '1.25rem', marginBottom: '1rem' }}>Upcoming Service</h2>
                ...
                <div className={styles.cardGray}>
                    <p className={styles.cardTitle} style={{ textTransform: 'none' }}>No upcoming services scheduled.</p>
                </div>
            )}
            ...
            <div className={styles.statsGrid}>
                <Link to="/customer/bookings" style={{ textDecoration: 'none' }}>
                    <div className={`${styles.card} ${styles.cardHover}`}>
                        <h3 className={styles.cardTitle} style={{ fontSize: '1.1rem', color: '#0f172a' }}>My Bookings</h3>
                        <p style={{ color: '#64748b' }}>Request service and view history.</p>
                    </div>
                </Link>
                <Link to="/customer/vehicles" style={{ textDecoration: 'none' }}>
                    <div className={`${styles.card} ${styles.cardHover}`}>
                        <h3 className={styles.cardTitle} style={{ fontSize: '1.1rem', color: '#0f172a' }}>My Vehicles</h3>
                        <p style={{ color: '#64748b' }}>Manage your vehicles.</p>
                    </div>
                </Link>
                <Link to="/customer/invoices" style={{ textDecoration: 'none' }}>
                    <div className={`${styles.card} ${styles.cardHover}`}>
                        <h3 className={styles.cardTitle} style={{ fontSize: '1.1rem', color: '#0f172a' }}>My Invoices</h3>
                        <p style={{ color: '#64748b' }}>View payment history.</p>
                    </div>
                </Link>
                <Link to="/customer/profile" style={{ textDecoration: 'none' }}>
                    <div className={`${styles.card} ${styles.cardHover}`}>
                        <h3 className={styles.cardTitle} style={{ fontSize: '1.1rem', color: '#0f172a' }}>My Profile</h3>
